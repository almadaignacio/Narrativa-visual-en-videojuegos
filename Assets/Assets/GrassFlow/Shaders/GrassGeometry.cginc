...
g2f CreateBladeFromSource(float2 rngPrimer, float instID, in VertexData lvd, v2g grassVert, uint realInstID) {
	...
	#if defined(DEFERRED) || defined(GF_USE_DITHER) || defined(GRASS_DEPTH)
	...
	float3 camRight = 
					...
					#endif
					UNITY_MATRIX_V[0]	
				  + UNITY_MATRIX_V[1].xyz * (RNG * 0.8f - 0.4f)
				  + UNITY_MATRIX_V[2].xyz * (RNG * 0.8f - 0.4f)
					;
	#endif
	...
	float finalHeight = GET_FINAL_HEIGHT(lvd) * (1 + (camDist - 1) * 0.025);

	#if defined(USE_PARAM_MAP)
	float slope = dot(lvd.norm, float3(0, 1, 0));
	slope = (slope - terrainSlopeThresh) * terrainSlopeFade;
	finalHeight *= saturate(slope);
	widthMult *= saturate(slope * 10);
	#endif
	...
	normHeight *= fracFade * alphaBlendo;
	#endif
	...
	lvd.vertex = localPos + worldPos + (surfWorldNormal * bladeOffset);
	...
	float3 topView = -UNITY_MATRIX_V[1].xyz * saturate(dot(-UNITY_MATRIX_V[2].xyz, -surfWorldNormal) - 0.5) * (-3 * topViewPush);
	
	tV += rippleForce * normHeight;
	tV += windAdd * normHeight;
	tV += topView * finalHeight;
	worldPos = tV;
	...
	//add an epsilon to depth because it was weirdly flickering where random spots would be somehow behind the depth pass???
	#if defined(DEFERRED)
	o.pos.z += 0.000001;
	#endif

	#if !defined(GRASS_DEPTH) && !defined(FORWARD_ADD) && !defined(DEFERRED)
	...
	o.selfShadowUV.xy += (noiseSamp - 0.5) * selfShadowWind * windMult;
	...
	o.selfShadowUV.z = pow(normHeight, 4);
		...
		float3 windTintAdd = float3(1, 1, 1) + windTint.rgb * windTint.a * noiseSamp * windMult;
	...
	#if defined(USE_SPECULAR) && defined(DEFERRED)
	float specHeightMult = lerp(aoValue, 1 + (noiseSamp * 0.2), specHeight);
	o.specVec = (lerp(specHeightMult, 1, normHeight));
	#endif
