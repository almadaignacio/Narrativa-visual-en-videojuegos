...
#define RndFloat3Offset(offset) float3(rndm(offset.xy), 0.5, rndm(offset.xy))
...
inline float3 Grass_SafeNormalize(float3 inVec)
{
    float dp3 = max(1.175494351e-38, dot(inVec, inVec));
    return inVec * rsqrt(dp3);
}

float3 GrassNormalizeNormalPerPixel(float3 normalWS)
{
#if defined(UNITY_NO_DXT5nm) && defined(GF_NORMAL_MAP)
    return Grass_SafeNormalize(normalWS);
#else
...
inline VertexData GetPosData(uint idx, uint offset){
	...
	#define unpack(val) (float4((val & 0xFF), (val & 0xFF00) >> 8, (val & 0xFF0000) >> 16, (val & 0xFF000000) >> 24) * unpackMult)
		...
		float4 unpackedData = unpack(c.data1[1]);
		g.norm = unpackedData.xyz * 2 - 1;
		g.typeParams = unpackedData.w;

		#if defined(USE_COLOR_MAP) || defined(USE_PARAM_MAP) || defined(USE_TYPE_MAP)
		float2 uv;
		uv.x = f16tof32(c.data1[0]); uv.y = f16tof32(c.data1[0] >> 16);
		SampleVertexData(g, uv);
		#endif
	#endif
...
#define GET_POS_VARIANCE_VERT(finalHeight, vert) VariateBalanced(RndFloat3Offset(vert), variance.x) * finalHeight
...
#if !defined(DEFERRED) && !defined(HDRP)
inline float3 GetMainLighting(float3 worldPos, float3 worldNormal, float4 clipPos) {
	...
	//float3 cookie = SampleMainLightCookie(worldPos);
	//lighting *= cookie;
	//#endif
