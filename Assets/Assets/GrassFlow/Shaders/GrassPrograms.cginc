...
g2f mesh_vertex_shader(v2g IN, uint instID : SV_InstanceID){
	...
	//dont care about this anymore, bad feature
	...
	#define SampleTex(textur, uv) tex2Dbias(textur, float4(uv.xy, 0, _GlobalMipBias.x))
#else
	#define SampleTex(texto, uv) tex2D(texto, uv)
#endif
...
FragmentOutput fragment_shader(g2f i) {
	...
	//return 0;
	half4 texCol = SampleTex(_MainTex, i.uv);

	#if !defined(NO_TRANSPARENCY)

	texCol.a = saturate(texCol.a * alphaMult);
	...
	i.normal.xyz = normalize(i.normal.xyz + (UNPACK_NORMAL(SampleTex(bumpMap, i.uv), normalStrength) - float3(0,0,1)));
	#endif
    ...
    float ssao = SampleAmbientOcclusion(GetNormalizedScreenSpaceUV(i.pos));
	ssao = lerp(ssao, 1, _AO);
	...
	float selfShadow = ambientCOShadow + (1.0 - ambientCOShadow) * (1 - saturate(SampleTex(_MainTex, i.selfShadowUV).a * 5));
	selfShadow = saturate(lerp(selfShadow, saturate(selfShadow * 3), i.selfShadowUV.z));
	lighting *= selfShadow;
	#endif


	texCol.rgb *= lighting;
	texCol = min(texCol * i.col, 2);
...
void fragment_shader(g2f i) {
	...
	#if defined(SEMI_TRANSPARENT)
		alpha = SampleTex(_MainTex, TRANSFORM_TEX(i.uv, _MainTex)).a;
	#endif
		...
		return half4(GrassNormalizeNormalPerPixel(i.normal.xyz), 0);
	#endif
}
#endif