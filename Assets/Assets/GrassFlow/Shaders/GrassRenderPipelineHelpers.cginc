...
#define UNPACK_NORMAL(n, scale) UnpackNormalWithScale(n, scale)
	...
	#define RWS worldPos - _WorldSpaceCameraPos.xyz
#else
	#define RWS worldPos
#endif
...
#if UNITY_REVERSED_Z
#define GetClipPos(worldPos, pos)  TransformWorldToHClip(float4(RWS, 1.0));\
...
#define GET_LIGHT_DIR(light) shadergraph_HDMainLightDirection()
#define UNPACK_NORMAL(n, scale) UnpackNormalMapRGorAG(n, scale * 0.25)
...
#if defined(GRASS_DEPTH) || defined(DEFERRED)
